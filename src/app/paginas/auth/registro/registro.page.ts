...
//------------ FORMULARIO PARA REGISTRO DE USUARIO ------------//
    form_registro = new FormGroup({
      ...
      displayName: string = '';           // Variable para almacenar el concatenado
      ...
      initializeDisplayName() {
        this.form_registro.valueChanges.subscribe((formValues) => {         // Escuchar cambios en firstname y lastname
 ...
 //------------ FUNCIÓN PARA SUSCRIBIR EL FORMULARIO ------------//
  async submit() {
    if (this.form_registro.valid) {                                 //  FORMULARIO VÁLIDO?
      ...
      this.firebaseSvc.signUp(userData as User)                     // PROMESA DE AUTENTICACIÓN CON LOS DATOS DE FORM_LOGIN COMO MODELO USER
