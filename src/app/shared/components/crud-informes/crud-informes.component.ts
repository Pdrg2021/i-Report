...
// ------------ DEFINICIÓN DE LOS CAMPOS DE FORMULARIO INFORME ------------  
  form_Informe = new FormGroup({
    ...
    nroAviso:       new FormControl(null , [Validators.required,
  ...
  //-----FUNCION: GENDERAR PDF DEL INFORME
    ...
    const docDefinition = {
      content: [
        ...
        {
          table: {
            widths: [80, 'auto', '*', 110, 'auto', '*'], 
            body: [
              [{text:'Instalación', fillColor: '#f0f0f0'},{text:':', fillColor: '#f0f0f0'}, formData.instalacion || '', {text:'Sistema:', fillColor: '#f0f0f0'},{text:':', fillColor: '#f0f0f0'}, formData.sistema || ''],
              [{text:'N° de PT', fillColor: '#f0f0f0'}, {text:':', fillColor: '#f0f0f0'},formData.nroPT || '', {text:'Responsable:', fillColor: '#f0f0f0'}, {text:':', fillColor: '#f0f0f0'},formData.responsable || ''],
              [{text:'N° de OT', fillColor: '#f0f0f0'}, {text:':', fillColor: '#f0f0f0'},formData.nroOT || '', {text:'Fecha de Ejecución:', fillColor: '#f0f0f0'},{text: ':', fillColor: '#f0f0f0'},formData.fechaejecucion || ''],
              [{text:'N° de Aviso', fillColor: '#f0f0f0'}, {text:':', fillColor: '#f0f0f0'},formData.nroAviso || '', {text:'Fecha de Informe:', fillColor: '#f0f0f0'},{text:':', fillColor: '#f0f0f0'}, formData.fechainforme || ''],
            ],
          },
        ...
        { 
          ...
          widths: ['*'], 
          body: [
              [{text:'Contexto y Antecedentes de lo encontrado. ', fillColor: '#f0f0f0'}],
              [formData.textoContexto || 'Sin información.'],
              [{text:' Antecedentes de lo realizado.  ', fillColor: '#f0f0f0'}],
              [ formData.textoRealizado || 'Sin información.'],
            ],
          },
        layout: 'lightVerticalLines',
        margin: [0, 0, 0, 10],
        },
        { text: 'Contexto y Antecedentes de lo encontrado. ', style: 'subheader', margin: [0, 10, 0, 5] },
        ...
        { text: 'Resumen tras Intervención', style: 'subheader', margin: [0, 10, 0, 5] },
        {
          table: {
            // widths: ['*'], 
            body: [
              ['Sí', formData.checkbox1  ? '( X )':'( _ )' ],
              ['No', formData.checkbox2  ? '( X )':'( _ )' ],
              ['Con Observaciones', formData.checkbox3  ? '( X )':'( _ )' ],
            ],
          },
          layout: 'noBorders',
          margin: [0, 0, 0, 10],
        },
        { text: formData.textoObservaciones || 'Sin Observaciones.', margin: [0, 0, 0, 10] },
        ...
        { text: 'Repuestos y Componentes', style: 'subheader', margin: [0, 10, 0, 5] },
        {
          table: {
            // widths: ['*'], 
            body: [
              ['Se realizó ajuste(s) de parámetro(s)', formData.checkbox4 ? '( Sí  )':'( No )' ],
              ['Se realizó reparación(es) de componente(s)', formData.checkbox5 ? '( Sí  )':'( No )' ],
              ['Se realizó reemplazo de componente(s)', formData.checkbox6 ? '( Sí  )':'( No )' ],
              ['Se implementó mejoras con nuevo(s) componente(s)', formData.checkbox7 ? '( Sí  )':'( No )' ],
            ],
          },
          layout: 'noBorders',
          margin: [0, 0, 0, 10],
        ...
        ...(imageBase64
          ...
          { 
            ...
            widths: ['*'], 
            body: [
                [{text:'Desfase de Informe (Fecha Reporte - Fecha Ejecución) ', fillColor: '#f0f0f0'}],
                [formData.desfase|| 'Sin información.'+ 'días de desfase'],
                [{text:'Edad de Informe (Fecha de Generación PDF - Fecha de Creación)  ', fillColor: '#f0f0f0'}],
                [ formData.edadReport || 'Sin información.' + 'Tiempo de Vida del Documento']
...
//----------       GENERADOR DEL ARCHIVO PDF     ----------
    pdfMake.createPdf(docDefinition)
    .download(`${formData.fechaejecucion || ''}_${formData.instalacion || ''}_${formData.sistema || ''}_OT-${formData.nroOT || ''}_Aviso_${formData.nroAviso || ''}.pdf`);
  }
